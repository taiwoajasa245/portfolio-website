export default function Skills() {
  return (
    <>
      <div className="w-full mx-5 mb-20 ">
        <p className="text-[25px] text-[#FDFDFD]">Skills</p>

        <div className="flex ">
          <div className={` h-[480px] w-[1px] bg-[#343434] mr-5 rounded `} />
          {/* Text beside the vertical line */}
          <div className="text-[#8E8E8E] text-[14px] mt-8">
            <p className="">
              a dedicated and passionate Backend Developer and App Developer
              with more than 2 years of experiened into a fulfilling career
              where I get to create and innovate every day.
            </p>
          </div>
        </div>
      </div>
    </>
  );
}
